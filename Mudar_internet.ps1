...
3 {
    try {
        $tempPath = [System.IO.Path]::Combine($env:TEMP, "dhcp.cmd")
...
"@
        ...
        $cmdContent | Out-File -FilePath $tempPath -Encoding UTF8

        Start-Process "cmd.exe" -ArgumentList "/c $tempPath" -Verb RunAs

    }
    catch {
        Write-Host "Erro ao executar o script. Detalhes do erro: $_" -ForegroundColor $corErro
    }
    Read-Host "Pressione [Enter] para sair"
}
